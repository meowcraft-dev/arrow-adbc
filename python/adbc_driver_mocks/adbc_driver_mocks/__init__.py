...
"""Low-level ADBC bindings for the mocks driver."""
...
class DatabaseOptions(enum.Enum):
    ...
    ACCOUNT = "adbc.mocks.sql.account"
    APPLICATION_NAME = "adbc.mocks.sql.client_option.app_name"
    #: specify the OKTAUrl to use for OKTA Authentication
    AUTH_OKTA_URL = "adbc.mocks.sql.client_option.okta_url"
    #: specify the token to use for OAuth or other forms of authentication
    AUTH_TOKEN = "adbc.mocks.sql.client_option.auth_token"
    #: Specify auth type to use for mocks connection based on
    #: what is supported by the mocks driver. Default is
    #: "auth_mocks" (use ValueAuth* consts to specify desired
    #: authentication type).
    AUTH_TYPE = "adbc.mocks.sql.auth_type"
    #: When true, the MFA token is cached in the credential manager. True by default
    #: on Windows/OSX, false for Linux
    CLIENT_REQUEST_MFA_TOKEN = "adbc.mocks.sql.client_option.cache_mfa_token"
    #: When true, the ID token is cached in the credential manager. True by default
    #: on Windows/OSX, false for Linux
    CLIENT_STORE_TEMP_CRED = "adbc.mocks.sql.client_option.store_temp_creds"
    ...
    CLIENT_TIMEOUT = "adbc.mocks.sql.client_option.client_timeout"
    DATABASE = "adbc.mocks.sql.db"
    DISABLE_TELEMETRY = "adbc.mocks.sql.client_option.disable_telemetry"
    HOST = "adbc.mocks.sql.uri.host"
    #: JWT expiration after timeout
    #: use format like http://pkg.go.dev/time#ParseDuration such as
    #: "300ms", "1.5s" or "1m30s". ParseDuration accepts negative values
    #: but the absolute value will be used.
    JWT_EXPIRE_TIMEOUT = "adbc.mocks.sql.client_option.jwt_expire_timeout"
    ...
    JWT_PRIVATE_KEY = "adbc.mocks.sql.client_option.jwt_private_key"
    ...
    JWT_PRIVATE_KEY_VALUE = (
        "adbc.mocks.sql.client_option.jwt_private_key_pkcs8_value"
    )
    #: a passcode to use with encrypted private keys for JWT authentication.
    JWT_PRIVATE_KEY_PASSWORD = (
        "adbc.mocks.sql.client_option.jwt_private_key_pkcs8_password"
    )
    ...
    KEEP_SESSION_ALIVE = "adbc.mocks.sql.client_option.keep_session_alive"
    LOGIN_TIMEOUT = "adbc.mocks.sql.client_option.login_timeout"
    OCSP_FAIL_OPEN_MODE = "adbc.mocks.sql.client_option.ocsp_fail_open_mode"
    PORT = "adbc.mocks.sql.uri.port"
    PROTOCOL = "adbc.mocks.sql.uri.protocol"
    REGION = "adbc.mocks.sql.region"
    #: request retry timeout EXCLUDING network roundtrip and reading http response
    #: use format like http://pkg.go.dev/time#ParseDuration such as
    #: "300ms", "1.5s" or "1m30s". ParseDuration accepts negative values
    #: but the absolute value will be used.
    REQUEST_TIMEOUT = "adbc.mocks.sql.client_option.request_timeout"
    ROLE = "adbc.mocks.sql.role"
    SCHEMA = "adbc.mocks.sql.schema"
    SSL_SKIP_VERIFY = "adbc.mocks.sql.client_option.tls_skip_verify"
    WAREHOUSE = "adbc.mocks.sql.warehouse"
    ...
    USE_HIGH_PRECISION = "adbc.mocks.sql.client_option.use_high_precision"


class StatementOptions(enum.Enum):
    """Statement options specific to the mocks driver."""
    ...
    #: Defaults to 10.
    PREFETCH_CONCURRENCY = "adbc.mocks.rpc.prefetch_concurrency"
...
def connect(
    ...
    """
    Create a low level ADBC connection to mocks.
