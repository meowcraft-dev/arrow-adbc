...
library = os.environ.get("ADBC_MOCKS_LIBRARY")
target = source_root.joinpath(
    "./adbc_driver_mocks/libadbc_driver_mocks.so"
).resolve()
if not library:
    if os.environ.get("_ADBC_IS_SDIST", "").strip().lower() in ("1", "true"):
        print("Building sdist, not requiring ADBC_MOCKS_LIBRARY")
    elif os.environ.get("_ADBC_IS_CONDA", "").strip().lower() in ("1", "true"):
        print("Building Conda package, not requiring ADBC_MOCKS_LIBRARY")
    elif target.is_file():
        print("Driver already exists (but may be stale?), continuing")
    else:
        raise ValueError("Must provide ADBC_MOCKS_LIBRARY")
else:
...
version = get_version("adbc_driver_mocks")
